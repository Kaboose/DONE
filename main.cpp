...
//Levels
typedef enum { MENU_LEVEL, HOME_LEVEL, LEVEL_ONE, LEVEL_TWO, LEVEL_THREE, LEVEL_FOUR } level_type;
...
SDL_Texture* ice_texture = NULL;
SDL_Texture* earth_texture = NULL;
SDL_Texture* wind_texture = NULL;
SDL_Rect tornado_rectangle = { 72, 176, 40, 40 };
...
public:
	...
	void addSpell(Spell::spell_type spell);
...
void Character::handleInput(SDL_Event event)
{
	if( event.type == SDL_KEYDOWN )
    {
		...
		case SDLK_e:
			...
			break;
		case SDLK_f:
			if (event.key.repeat == 0 && mana > 0 && !casting && magicCapacity > 0)
			{
				SDL_RendererFlip flip;
				float xvel = 0;
...
void Character::addSpell(Spell::spell_type spell)
{
	magicArray[magicCapacity] = spell;
	magicCapacity++;
}
...
void Character::load_player()
{
	...
	for (int i = 0; i < magicCapacity; i++)
	{
		...
		{
		case 0:
			magicArray[i] = Spell::FIRE;
			break;
		case 1:
			magicArray[i] = Spell::ICE;
			break;
		case 2:
			magicArray[i] = Spell::EARTH;
			break;
		case 3:
			magicArray[i] = Spell::WIND;
			break;
		}
	}
}
...
{
public:
	typedef enum { STANDING_LEFT, STANDING_RIGHT, RUNNING_LEFT, RUNNING_RIGHT, JUMPING, SEQUENCE, RECOVER, DEAD} states;
	typedef enum { LEFT = 0, RIGHT, BOTTOM } sides;
...
//Boss Class and Functions
#pragma region Boss
class Boss : public Enemy
{
public:
	Boss(SDL_Texture *texture, int x, int y, types type);
	void update();
	void setState(states state);
...
Boss::Boss(SDL_Texture *texture, int x, int y, types type) : Enemy(texture, x, y, type)
{
	this->type = type;
	...
	instructions[0].state = RUNNING_LEFT;
	instructions[0].length = 200;
	instructions[1].state = RUNNING_RIGHT;
	instructions[1].length = 200;
}
...
void Boss::update()
{
	if (state == SEQUENCE)
	{
		if (frame - beginFrame < instructions[sequence_index].length)
			play_sequence();
		else
		{
			beginFrame = frame;
...
void Boss::play_sequence()
{
	if (instructions[sequence_index].state == STANDING_LEFT || instructions[sequence_index].state == STANDING_RIGHT || 
		instructions[sequence_index].state == RECOVER)
	{
		if (type == GRIZZOLAR_BEAR || type == SMALL_ORC)
		{
			if ((frame-beginFrame) % 6 == 0)
	...
	if (instructions[sequence_index].state == RUNNING_LEFT || instructions[sequence_index].state == RUNNING_RIGHT)
	{
		switch (type)
		{
		case SMALL_ORC:		
		...
		case LIZARD:
			...
			{
				int speed;
				(instructions[sequence_index].state == RUNNING_LEFT ? speed = -10 : speed = 10);
				fireball.init(fire, box.x, box.y, speed, 0, flip, Spell::FIRE, NULL);
				lastFired = frame;
				firing = true;
			}
		...
		if (instructions[sequence_index].state == RUNNING_LEFT)
		{
			xvel = -X_VELOCITY;
			if (collision[LEFT])
			{
		...
		if (instructions[sequence_index].state == RUNNING_LEFT || instructions[sequence_index].state == STANDING_LEFT)
			flip = SDL_FLIP_HORIZONTAL;
		else
			flip = SDL_FLIP_NONE;
	}
	...
	if (type != LIZARD)
	{
		box.w = 2*clip[currentClip].w;
		box.h = 2*clip[currentClip].h;
	}
...
void Boss::setState(states state)
{
	this->state = RECOVER;
	beginFrame = frame;
	sequence_index = 0;
}
#pragma endregion Boss Class
...
void EnemyManager::addBoss(int x, int y, Enemy::types type)
{
	SDL_Texture *texture = NULL;
	...
	bosses.push_back(Boss(texture, x, y, type));
}
...
void EnemyManager::update()
{
	...
	{
		Boss* boss = &bosses.at(i);

		collisionManager(boss);

		if (cameraCollision(boss->getBox()) && (Camera.x = LevelWidth - Camera.w))
			BOSS_FIGHT = true;
		if (BOSS_FIGHT)
			boss->update();
	}
...
void EnemyManager::draw()
{
	...
	for (int i = 0; i < bosses.size(); i++)
	{
		if (cameraCollision(bosses.at(i).getBox()))
			bosses.at(i).draw();
	}
}
	...
	if ( (star || sword || spell) && enemy->getState() != Enemy::RECOVER)
	{
		...
		else if (spell)
		{
			if (checkWeak(enemy->type, Player.spellType()))
			{
				...
				if (enemy->isBoss)
					enemy->health -= 3*Player.spellDamage();
			}
			else
...
void EnemyManager::collisionManager(Boss* enemy)
{
	if (enemy->getState() == Enemy::DEAD)
		return;
...
private:
	...
	std::vector<std::string> intro_script;
	std::vector<std::string> earth_script;
	std::vector<std::string> fire_script;
	std::vector<std::string> ice_script;
	std::vector<std::string> wind_script;
...
void AIManager::load()
{
	...
	//Scripts
	input_file.open("Media/Scripts/introduction.txt");
	if (input_file.is_open())
	{
		while (!input_file.eof())
		{
			...
			intro_script.push_back(line);
		}
	}

	input_file.close();

	input_file.open("Media/Scripts/Earth Script.txt");
	if (input_file.is_open())
	{
		while (!input_file.eof())
		{
			std::string line;
			getline(input_file, line);
			earth_script.push_back(line);
		}
	}

	input_file.close();

	input_file.open("Media/Scripts/Fire Script.txt");
	if (input_file.is_open())
	{
		while (!input_file.eof())
		{
			std::string line;
			getline(input_file, line);
			fire_script.push_back(line);
		}
	}

	input_file.close();

	input_file.open("Media/Scripts/Ice Script.txt");
	if (input_file.is_open())
	{
		while (!input_file.eof())
		{
			std::string line;
			getline(input_file, line);
			ice_script.push_back(line);
		}
	}

	input_file.close();

	input_file.open("Media/Scripts/Wind Script.txt");
	if (input_file.is_open())
	{
		while (!input_file.eof())
		{
...
void AIManager::addAI(int x, int y, AI::types type)
{
	...
	{
	case HOME_LEVEL:
		bots.push_back(AI(x, y, wizard_texture, AI::WIZARD, intro_script));
		break;
	case LEVEL_ONE:
		bots.push_back(AI(x, y, wizard_texture, AI::WIZARD, earth_script));
		break;
	case LEVEL_TWO:
		bots.push_back(AI(x, y, wizard_texture, AI::WIZARD, fire_script));
		break;
	case LEVEL_THREE:
...
public:
	Door(int x, int y, SDL_Texture* texture, SDL_Texture* element_texture, level_type level);
	SDL_Rect getBox();
	void update();
	void draw();
};

Door::Door(int x, int y, SDL_Texture* texture, SDL_Texture* element_texture, level_type level)
{
	...
	this->texture = texture;
	this->element_texture = element_texture;
	this->level = level;
}
...
void Door::draw()
{
	SDL_Rect destination = { box.x - Camera.x, box.y - Camera.y, box.w, box.h };
	SDL_RenderCopy(renderer, texture, &clip, &destination);
	SDL_Rect tempMagic = { box.x+box.w/2 - 20 - Camera.x, box.y - 40 - Camera.y, 40, 40 };
	if (element_texture == wind_texture)
		SDL_RenderCopy(renderer, element_texture, &tornado_rectangle, &tempMagic);
	else
		SDL_RenderCopy(renderer, element_texture, NULL, &tempMagic);
}
...
void DoorManager::load()
{
	...
	fire_texture = loadImage("Media/Objects/fireball.png");
	ice_texture = loadImage("Media/Objects/snowflake.png");
	earth_texture = loadImage("Media/Objects/rock.png");
	wind_texture = loadImage("Media/Objects/magic_icons.png");
}
...
void DoorManager::addDoor(int x, int y)
{
	...
	{
	case 0:
		element = earth_texture;
		level = LEVEL_ONE;
		break;
	case 1:
		element = fire_texture;
		level = LEVEL_TWO;
		break;
	case 2:
		element = wind_texture;
		level = LEVEL_THREE;
		break;
	case 3:
		element = ice_texture;
	...
	doors.push_back(Door(x, y, texture, element, level));
	num_doors++;
}
...
private:
	...
	Tile *home_level[360];
	textures home_level_blueprint[360];
...
#pragma endregion tiles
		...
		case BOR:
			level = NULL;
			EnemyMang.addBoss((i%width)*tex_width, (i/width)*tex_height, Enemy::SMALL_ORC);
			break;
		case BOB:
			level = NULL;
			EnemyMang.addBoss((i%width)*tex_width, (i/width)*tex_height, Enemy::GRIZZOLAR_BEAR);
			break;
		case BIZ:
			level = NULL;
			EnemyMang.addBoss((i%width)*tex_width, (i/width)*tex_height, Enemy::LIZARD);
			break;
		case BBR:
			level = NULL;
			EnemyMang.addBoss((i%width)*tex_width, (i/width)*tex_height, Enemy::BIRD);
			break;
		case WIZ:
			level = NULL;
...
#pragma region Conversions
	if (enumString == "NE1")
		...
		return BIR;
	else if (enumString == "BOR")
		return BOR;
	else if (enumString == "BOB")
		return BOB;
	else if (enumString == "BIZ")
		return BIZ;
	else if (enumString == "BBR")
		return BBR;
